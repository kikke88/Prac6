...
void quantum_transformation(const complexd* a, complexd* result, const int n, const int num_of_elem, const double u[][2], const int k, const double sum)
{
	int test = 1 << (n - k);
#pragma omp parallel for default(none) shared(a, u, result, test)
	for (int i = 0; i < num_of_elem; i += 1) {
		result[i] = (u[i & test && 1][0] * a[i & ~(test)] + u[i & test && 1][1] * a[i | test]) / sum;
	}
/*
...
#pragma omp parallel  default(none) shared(qbit_vec, num_of_elem, sum) private(thrd_seed)
		{
			thrd_seed = omp_get_thread_num();
#pragma omp for reduction(+:sum)
		...
		quantum_transformation(qbit_vec, result, n, num_of_elem, matrix, k, sum);
		prog_time2 += omp_get_wtime();
	}
	delete[] qbit_vec;
	delete[] result;
