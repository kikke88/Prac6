...
public:
    ...
    const complexd* get_vector_1();
    const int one_proc_size();
};
...
void Quantum_vector::single_qubit_transform(const int k, const complexd matrix[4])
{
...
{
    const complexd P_S[4] {complexd(1, 0), complexd(0, 0), complexd(0, 0), complexd(cos(phi), sin(phi))};
    single_qubit_transform(k, P_S);
}

void Quantum_vector::NOT_gate(const int k)
{
    const complexd NOT[4] {complexd(0, 1), complexd(1, 0), complexd(1, 0), complexd(0, 0)};
    single_qubit_transform(k, NOT);
}

void Quantum_vector::double_qubits_transform(const int k, const int l, const complexd matrix[16])
{
    int log_size {static_cast<int>(log2(size))};
    ...
    if (k > log_size && l > log_size) {
        ...
        for (int i = 0; i < elem_in_one_proc; ++i) {
            if (k < l) {
                ...
                i_k_i_l = (i & test_l && 1) << 1 | (i & test_k && 1);
            }
            vector_2[i] = matrix[i_k_i_l] * vector_1[i & ~(test_k | test_l)] +
                          matrix[4 | i_k_i_l] * vector_1[i & ~test_k | test_l] +
                          matrix[8 | i_k_i_l] * vector_1[i & ~test_l | test_k] +
                          matrix[12 | i_k_i_l] * vector_1[i | test_k | test_l];
        }
...
#pragma omp parallel for
            ...
            i_k_i_l = 3;
            int dest_rank_0 {(rank ^ 1 << (log_size - l)) ^ 1 << (log_size - k)};
            int dest_rank_1 {rank ^ 1 << (log_size - k)};
            int dest_rank_2 {rank ^ 1 << (log_size - l)};
...
{
    const complexd CNOT[16] {complexd(1, 0), complexd(0, 0), complexd(0, 0), complexd(0, 0),
...
void Quantum_vector::Controlled_Phase_shift_gate(const int k, const int l, const double phi)
{
    const complexd C_P_S[16] {complexd(1, 0), complexd(0, 0), complexd(0, 0), complexd(0, 0),
                              complexd(0, 0), complexd(1, 0), complexd(0, 0), complexd(0, 0),
                              complexd(0, 0), complexd(0, 0), complexd(1, 0), complexd(0, 0),
                              complexd(0, 0), complexd(0, 0), complexd(0, 0), complexd(cos(phi), sin(phi))};
...
const complexd* Quantum_vector::get_vector_1()
{
    return vector_1;
}

const int Quantum_vector::one_proc_size()
{
    return elem_in_one_proc;
}
