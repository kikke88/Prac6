...
int main(int argc, char* argv[])//n file1 file2
{
	int n = strtol(argv[1], NULL, 10),
		num_of_elem = 1 << n;
	//std::cout << 10e10 * DBL_EPSILON << std::endl;
	...
	for (int i = 0; i < num_of_elem; ++i) {
		//std::cout << qbit_vec1[i] << std::endl << qbit_vec2[i] << std::endl;
		if (abs(qbit_vec1[i].real() - qbit_vec2[i].real()) > 10e10 * DBL_EPSILON or
            abs(qbit_vec1[i].imag() - qbit_vec2[i].imag()) > 10e10 * DBL_EPSILON) {
